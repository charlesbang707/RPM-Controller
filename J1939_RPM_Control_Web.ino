...
// CAN 초기화 함수 (에러 해결: if-else 구조로 변경)
...
void CAN_Loop(void * pvParameters) {
    ...
    for(;;) {
        ...
        if (status.state == TWAI_STATE_BUS_OFF) initCAN(is500k);
...
class MyCharCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pChar) {
        ...
        if (v.length() >= 2) {
            ...
            if (val == 0x5000) { is500k = true; initCAN(true); sendAddressClaim(); }
            else if (val == 0x2500) { is500k = false; initCAN(false); sendAddressClaim(); }
            else if (val == 1000 && currentState == IDLE) { currentState = RUNNING; targetRPM = 1000; }
